...
public class FamiliarsSavedData extends SavedData {

    // Primary map: mob UUID → owner player UUID (serialized)
    private final Map<UUID, UUID> mobToPlayer = new HashMap<>();
    // Inverse index: player UUID → list of mob UUIDs (rebuilt from primary, not serialized)
    ...
    private static final Codec<FamiliarsSavedData> CODEC = Bond.CODEC.listOf().xmap(
        bonds -> {
            FamiliarsSavedData d = new FamiliarsSavedData();
            for (Bond b : bonds) {
                try {
    ...
    private static final SavedDataType<FamiliarsSavedData> TYPE = new SavedDataType<>(
        Identifier.fromNamespaceAndPath("darknights", "familiars"),
        FamiliarsSavedData::new,
        CODEC,
        null
    );

    public FamiliarsSavedData() {}

    public static FamiliarsSavedData get(MinecraftServer server) {
    ...
    public void attach(UUID mobUuid, UUID playerUuid) {
        detach(mobUuid); // clean up any prior bond
